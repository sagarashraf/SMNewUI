...
module.exports = async function PhysicallyActive(option, sex) {
	switch (option) {
		case 1:
			yesQuery = `select impact, life_exp_val from phys_active where gender= "${sex}" and option_ = "1"`;
			let yesStateImpact = await JsonConverter(await SqlQueryHandler(yesQuery));

			return yesStateImpact;
		case 0:
			noQuery = `select impact, life_exp_val from phys_active where gender= "${sex}" and option_ = "0"`;
