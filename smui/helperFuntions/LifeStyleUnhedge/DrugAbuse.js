...
module.exports = async function DrugAbuse(option, sex) {
	switch (option.state) {
		case 1:
			yesQuery = `select impact from alcohol_abuse where gender= "${sex}" and option_ = "1"`;
			let yesStateImpact = await JsonConverter(await SqlQueryHandler(yesQuery));
			let type = `select impact from alcohol_abuse_options where gender= "${sex}" and option_ = "${option.type}" `;
			let typeImpact = await JsonConverter(await SqlQueryHandler(type));
			return [yesStateImpact[0].impact];
		case 0:
			noQuery = `select impact from alcohol_abuse where gender= "${sex}" and option_ = "0"`;
