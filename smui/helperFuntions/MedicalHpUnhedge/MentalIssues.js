...
module.exports = async function MentalIssues(object, sex) {
	switch (object.state) {
		case 1:
			yesQuery = `select impact, life_exp_val from mental_issues where gender= "${sex}" and option_ = "1"`;
			let yesStateImpact = await JsonConverter(await SqlQueryHandler(yesQuery));

			yearsLevelQuery = `select impact, life_exp_val from mental_issues_years where gender= "${sex}" and option_ =${object.level}`;
