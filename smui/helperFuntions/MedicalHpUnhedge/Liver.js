...
module.exports = async function Liver(object, sex) {
	switch (object.state) {
		case 1:
			yesQuery = `select impact from liver where gender= "${sex}" and option_ = "1"`;
			...
			let type = `select imapct from liver_options where gender= "${sex}" and option_ = "${cancerobject.type}" `;
		...
		case 0:
			noQuery = `select impact from liver where gender= "${sex}" and option_ = "0"`;
