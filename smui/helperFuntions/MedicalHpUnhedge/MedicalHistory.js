...
module.exports = async function MedicalHistory(option, sex) {
	switch (option) {
		case "yes":
			yesQuery = `select impact from medical_history where gender= "${sex}" and option_ = "yes"`;
			let yesStateImpact = await JsonConverter(await SqlQueryHandler(yesQuery));
			return [yesStateImpact[0].impact];

		case "no":
			noQuery = `select impact from medical_history where gender= "${sex}" and option_ = "no"`;
