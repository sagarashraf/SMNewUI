...
module.exports = async function NeurologicalDis(object, sex) {
	switch (object.state) {
		case 1:
			yesQuery = `select impact, life_exp_val from neuro_disorder where gender= "${sex}" and option_ = "1"`;
			let yesStateImpact = await JsonConverter(await SqlQueryHandler(yesQuery));

			yearsLevelQuery = `select impact, life_exp_val from neuro_disorder_years where gender= "${sex}" and option_ =${object.level}`;
			let yearLevelImpact = await JsonConverter(
				await SqlQueryHandler(yearsLevelQuery)
			);
			let type = `select impact, life_exp_val, mortality_val from neuro_disorder_options where gender= "${sex}" and option_ = "${object.type}" `;
		...
		case 0:
			noQuery = `select impact, life_exp_val from neuro_disorder where gender= "${sex}" and option_ = "0"`;
